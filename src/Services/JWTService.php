...
class JWTService
{
    ...
    {
        // $_SESSION['ds_expiration'] = time(); // AK: this is how to reset token if something wrong.
    ...
    private function configureJwtAuthorizationFlowByKey()
    {
        ...
        $privateKey = file_get_contents(
            /*$_SERVER['DOCUMENT_ROOT'] . '/' . */$GLOBALS['JWT_CONFIG']['private_key_file'], // AK
            true
        );
    ...
    function authCallback($redirectUrl): void
    {
        ...
        if (!self::$access_token) {
            ...
            try {
                ...
                $_SESSION['ds_expiration'] = time() + (self::$access_token->getExpiresIn() /** 60*/); # expiration time. // AK: return value is in seconds already. No need to multiply to 60.
